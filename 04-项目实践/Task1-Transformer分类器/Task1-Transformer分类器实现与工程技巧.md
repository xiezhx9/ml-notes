...
1. 为什么使用公开 tokenizer 不等于使用预训练 BERT？
	1. 还需要使用配套embedding 模型参数

> [!answer] 参考答案
> 这里需要纠正一点：公开 tokenizer 只负责分词，并根据它维护的词表把 token 映射成整数 ID；它不包含 Transformer 的 Embedding、注意力层或其他预训练参数。本项目可以按照该词表大小随机初始化 `nn.Embedding`，再从分类任务中学习，并不强制使用配套的预训练 Embedding。只有想复用 BERT 的预训练语义时，才需要加载与 tokenizer 词表和模型结构相匹配的预训练权重。

2. `padding_idx` 已经让 PAD Embedding 为零，为什么仍需要 attention mask？
	1. 添加位置编码后padding的特征向量会改变，所以需要attention mask来标记

> [!answer] 参考答案
> 你的答案抓住了关键原因。`padding_idx` 只保证 PAD 对应的词向量为零且该行不被梯度更新，但加上位置编码、经过带偏置的线性层后，PAD 位置仍可能变成非零；即便 Key 是零，它也会进入 softmax 分母并分走概率。因此 attention mask 要阻止有效 token 读取 PAD，pooling mask 还要阻止 PAD 参与句向量平均。

3. 为什么第 2 层注意力可视化必须先运行第 1 层 Block？

> [!answer] 参考答案
> 第 2 层的真实输入不是原始 Embedding，而是第 1 层 Block 的输出。只有先按模型顺序运行第 1 层，才能得到训练和推理时第 2 层真正接收的隐藏状态；若直接把 Embedding 喂给第 2 层，画出的只是一个模型实际从未执行过的虚假计算路径。

4. `transpose` 后为什么 `reshape` 可用，而 `view` 可能失败？

> [!answer] 参考答案
> `transpose` 通常只改变张量的 stride，不会重新排列底层存储，因此结果往往不连续。`view` 要求当前内存布局能够直接解释为目标形状，遇到这种非连续布局可能报错；`reshape` 会在可行时返回 view，不可行时自动复制为连续存储。另一种明确写法是 `x.transpose(...).contiguous().view(...)`。

5. 为什么验证 loss 要乘当前 batch size 后再累加？

> [!answer] 参考答案
> `CrossEntropyLoss` 默认返回当前 batch 的平均 loss。先乘当前 batch size 可以还原该 batch 的样本 loss 总和，全部累加后再除以验证集总样本数，才能得到按样本加权的整体平均值。若直接平均各 batch 的平均 loss，最后一个较小 batch 会与完整 batch 拥有相同权重，从而产生偏差。

6. `clip_grad_norm_` 的返回值是裁剪前还是裁剪后范数？
	1. 裁剪后的

> [!answer] 参考答案
> 这里需要纠正：它返回的是**裁剪前**所有参数梯度的总范数，然后在原地按需要缩放梯度。如果该范数超过 `max_norm`，梯度会统一乘上近似 $\text{max\_norm}/\text{total\_norm}$ 的系数。因此这个返回值很适合记录“原始梯度有多大”，而真正用于 `optimizer.step()` 的是裁剪后的梯度。

7. 如果采集注意力后忘记恢复 `model.train()`，训练会发生什么变化？

> [!answer] 参考答案
> 模型会继续停留在 eval 模式，后续训练中的 Dropout 将被关闭，模型失去这部分随机正则化，训练行为与原设计不一致；若含 BatchNorm，其运行统计量也不会继续更新。梯度本身仍可计算，因为 `eval()` 并不等于 `no_grad()`，所以这个错误可能不会报错，却会悄悄改变训练结果。

8. 为什么动画每一帧必须共享颜色范围？

> [!answer] 参考答案
> 共享 `vmin/vmax` 后，同一种颜色在所有 epoch 都代表相同的注意力数值，才能比较权重的绝对变化。若每帧各自自动缩放，即使某一帧的最大权重很小，也会被显示成最亮颜色，从而制造注意力“增强”或“同样强”的视觉错觉。

9. 为什么最终静态热图可能和演化图最后一行不一致？

> [!answer] 参考答案
> 当前实现中，演化图最后一行保存的是最后一个 epoch 的模型状态，而最终静态热图从验证 loss 最优的 checkpoint 生成。如果最佳 epoch 不是最后一个 epoch，两幅图对应的参数状态就不同，因此注意力也可能不同。

10. 当前 mean pooling 设计下，只看 `[CLS]` 注意力是否足以解释分类？
	1. 注意力没法解释分类，只关心信息的分配

> [!answer] 参考答案
> 不足以。你的方向是对的：注意力主要展示信息如何被读取，不能直接当作完整的分类归因；而且当前分类器对所有有效 token 的最终隐藏状态做 mean pooling，并不是只把 `[CLS]` 向量送进分类头。因此分类结果由全部有效 token 共同参与，只观察 `[CLS]` 那一行会漏掉其他 Query 对句向量的贡献。
