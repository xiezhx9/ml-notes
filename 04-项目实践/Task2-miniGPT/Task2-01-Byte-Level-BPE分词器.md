...
1. 为什么 byte-level BPE 没有传统 OOV？
	1. 任意语言的任意词语在计算机内任意编码格式本质都是用字节来表达，无非字节多少，所以字节对可以表示所有词句
2. 为什么 merge rule 的执行顺序必须保存？
	1. 因为后续encode文本的时候需要按这个顺序去合并字节，得到最后的分词结果
3. 为什么 pair 相等要比较 tuple，而不能只比较拼接结果？
	1. 每一轮需要保存合并的次序；aa,b a,ab本质是两种合并词序，比较拼接结果容易混为一谈
4. 为什么 decode 要先拼 bytes，再做 UTF-8 解码？
	1. bpe内部存储的词表是以byte对作为基础的，token id 转换为对应的字节对之后可以做utf-8解码
5. 缩小词表会同时带来哪些好处和代价？
	1. 小词表减少输入/输出层参数并提高 token 频次，但会让序列更长、token变小一个句子会包含更多token，Attention 计算量级更大。

> [!note] 自测批改
> 1. **基本正确。** 更严谨地说，本实现先把文本编码为 UTF-8 bytes，256 个基础 byte token 就能覆盖所有合法 UTF-8 文本；这里应说“byte”，而不是“字节对”。
> 2. **方向正确，但少了核心原因。** encode 确实要重放规则；更根本的是后期 merge 的输入符号可能由前期 merge 产生，因此规则之间存在依赖，交换顺序可能改变结果。
> 3. **正确。** 重点不是拼接后的 bytes，而是当前符号序列中的 token 边界：`(a, ab)` 与 `(aa, b)` 是两种不同的相邻 pair。
> 4. **方向正确，但需要补充“为什么”。** 一个 Unicode 字符的 UTF-8 bytes 可能分散在多个 token 中，单独解码某个 token 可能是不完整字节序列；必须先还原并拼接全部 bytes，再统一解码。
> 5. **正确。** 再补充一点：Attention 对序列长度通常是二次复杂度，因此 token 变多可能明显增加计算和显存开销。
